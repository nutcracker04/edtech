...
async def extract_questions_from_image(image_bytes: bytes) -> List[ExtractedQuestion]:
    """
    ...
    try:
        ...
        questions = parse_questions(full_text, ocr_data)
...
def parse_questions(text: str, ocr_data: Dict[str, Any]) -> List[ExtractedQuestion]:
    """
        ...
        if question_match:
            ...
            if current_question:
                questions.append(ExtractedQuestion(
                    ...
                    confidence=calculate_average_confidence(ocr_data, i),
    ...
    if current_question:
        questions.append(ExtractedQuestion(
            ...
            question_text=current_question,
            options=[{"id": opt[0], "text": opt[1]} for opt in current_options],
            confidence=calculate_average_confidence(ocr_data, len(lines) - 1),
            subject=None,
            topic=None
        ))
...
def calculate_average_confidence(ocr_data: Dict[str, Any], line_index: int) -> float:
    """
...
async def classify_question_subject(question_text: str) -> str:
    """
